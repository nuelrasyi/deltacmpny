...
import { Plus, Search, MoreVertical, BookOpen, Edit, Trash2 } from 'lucide-react'
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Kelola seluruh program pelatihan yang tersedia.</p>
        </div>
        <Link href="/superadmin/programs/new" className="flex items-center px-4 py-2.5 bg-primary-600 text-white rounded-xl text-sm font-medium hover:bg-primary-700 transition-all shadow-sm hover:shadow-md hover:shadow-primary-500/20">
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <p className="text-sm font-medium text-slate-900">Belum ada program</p>
                      <p className="text-sm text-slate-500 mt-1">Mulai dengan menambahkan program pelatihan pertama Anda.</p>
                    </div>
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={program.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500">{program.slug}</p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${
                        ...
                        <span className={`w-1.5 h-1.5 rounded-full mr-1.5 ${program.is_active ? 'bg-emerald-500' : 'bg-slate-400'}`}></span>
                        {program.is_active ? 'Aktif' : 'Nonaktif'}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex justify-end gap-2">
                        <Link href={`/superadmin/programs/${program.id}/edit`} className="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
